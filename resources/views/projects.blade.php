<x-guest-layout>
            ...
            <form action="{{ route('projects') }}" method="post" class="flex flex-col hidden border border-x-black border-b-black rounded-b" id="filter">
                ...
                @foreach($languages as $language)
                    <div class="flex flex-row justify-between">
                        <input type="checkbox" @if (in_array($language->id, $selected)) checked @endif name="language[]" class="rounded my-auto ml-1" id="{{ $language->id }}" value="{{ $language->id }}">
    ...
    <section class="w-4/5 h-auto flex mx-auto mt-8 flex-wrap justify-evenly">
        @foreach($projects AS $project)
            <div class="max-w-sm rounded overflow-hidden shadow-lg bg-white w-full sm:w-2/5 md:w-3/6 mr-px mb-6 sm:mb-16">
